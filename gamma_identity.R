mlg_estGID=function(Y,X,Nc,K,alpha,phi){
 X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = Gamma(link="identity"))
  ...
  while((criterio>1e-4)&&(cont<1000)){
   ...
   I_beta2=t(X)%*%W%*%X                                           
    ...
    dev=ifelse(ycond,
    as.numeric(2*phi*(sum(Y/(1+Y)+log(mu)+Y/mu))),
    as.numeric(2*sum(-log(Y/mu)+(Y-mu)/mu))) 
    
    phi=as.numeric((n-p)/(t((Y-mu)/mu)%*%((Y-mu)/mu)))
...
CVspalphaGID<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estGID(Y,X,Nc,K,alpha,phi)
