mlg_estBL=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  Y=as.matrix(Y)
  Yv=Y
  q1=ncol(Nc)
  ...
  stop("numero de colunas de Y maior que 2")
   ...
   ob=glm(Yv~X,family = binomial(link="logit"))
...
CVspalphaBL<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estBL(Y,X,Nc,K,alpha,phi)
