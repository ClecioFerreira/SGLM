mlg_estBCLL=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  Y=as.matrix(Y)
  Yv=Y
  q1=ncol(Nc)
  ...
  if(ncol(Y)==1) {
   ...
   ob=glm(Yv~X,family = binomial(link="cloglog"))
  ...
  phi=n
  mu=1-exp(-exp(eta))
    W=diag((1-mu)*(log(1-mu)^2)/mu)        
    V=diag(mu*(1-mu))
    Wm=diag(sqrt((1-mu)*(log(1-mu)^2)/mu))
...
CVspalphaBCLL<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estBCLL(Y,X,Nc,K,alpha,phi)
