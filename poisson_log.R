mlg_estPL=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = poisson(link="log"))
...
CVspalphaPL<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estPL(Y,X,Nc,K,alpha,phi)
