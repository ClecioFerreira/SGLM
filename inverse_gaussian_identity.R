mlg_estIGID=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = inverse.gaussian(link = "identity"))
...
CVspalphaIGID<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estIGID(Y,X,Nc,K,alpha,phi)
