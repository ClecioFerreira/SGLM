mlg_estBP=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  Y=as.matrix(Y)
  Yv=Y
  q1=ncol(Nc)
  ...
  ob=glm(Yv~X,family = binomial(link="probit"))
  ...
  phi=n
  mu=qnorm(eta)
    W=diag(dnorm(qnorm(mu))^2/(mu*(1-mu)))
   V=diag(mu*(1-mu))
   Wm=diag(sqrt(dnorm(qnorm(mu))^2/(mu*(1-mu))))
...
CVspalphaBP<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estBP(Y,X,Nc,K,alpha,phi)
