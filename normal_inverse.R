mlg_estNIV=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = gaussian(link="inverse"))
  ...
  while((criterio>1e-4)&&(cont<1000)){
    ...
    mu=1/eta
   W=diag(as.vector(1/(eta^4)),ncol=n)
    Wm=diag(sqrt(1/(eta^4)))
...
CVspalphaNIV<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
   param=mlg_estNIV(Y,X,Nc,K,alpha,phi)
