mlg_estPSQRT=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = poisson(link="sqrt"))
...
while((criterio>1e-4)&&(cont<1000)){
    ...
    dev=as.numeric(2*(t(Y)%*%(log(Y)-log(mu))-sum(Y-mu)))
    #dev=2*sum(Y*log(Y/mu)-(Y-mu))   #duas formas de escrever a mesma dev 
                                  
    criterio=as.numeric(sum(abs(dev-dev0))) 
...
CVspalphaPSQRT<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
param=mlg_estPSQRT(Y,X,Nc,K,alpha,phi)
