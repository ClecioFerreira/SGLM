mlg_estNID=function(Y,X,Nc,K,alpha,phi){
  X=as.matrix(X)
  q1=ncol(Nc)
  p=ncol(X)
  lteta=p+q1
  n=length(Y)
  ob=glm(Y~X,family = gaussian(link="identity"))
...
CVspalphaNID<-function(alpha,Y,X,t,Nc,K,phi,method,ndx){
  p=ncol(X)
   n=length(Y)
  param=mlg_estNID(Y,X,Nc,K,alpha,phi)
  ...
  f=theta[(p+1):length(theta)]
   ...
   H=cbind(X,Nc)%*%solve(MI, tol=1e-23)%*%rbind(t(X),t(Nc))%*%W
   trH=sum(diag(H))
